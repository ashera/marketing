...
[`prompt-log.md`](prompt-log.md) with: **title, type + model, ISO 8601 datetime,
...
recorded in [`ref-ids.md`](ref-ids.md) as **UUID → local file**. It's the single
...
`scripts/generate.ps1` enforces this (estimate by default; `-Go` to render).
...
[README](README.md) for repo structure.
